package com.docbase.admin.controller.common;
...
import com.docbase.domain.common.dto.TokenDTO;
import com.docbase.domain.system.menu.MenuApplicationService;
...
import com.docbase.infrastructure.annotations.ratelimit.RateLimit;
import com.docbase.infrastructure.annotations.ratelimit.RateLimit.CacheType;
import com.docbase.infrastructure.annotations.ratelimit.RateLimit.LimitType;
import com.docbase.infrastructure.user.AuthenticationUtils;
import com.docbase.admin.customize.service.login.dto.CaptchaDTO;
import com.docbase.admin.customize.service.login.dto.ConfigDTO;
import com.docbase.admin.customize.service.login.command.LoginCommand;
import com.docbase.infrastructure.user.web.SystemLoginUser;
import com.docbase.infrastructure.annotations.ratelimit.RateLimitKey;
...
public class LoginController {
    ...
    private final DocBaseConfig docBaseConfig;
    ...
    public String index() {
        return StrUtil.format("欢迎使用{}后台管理框架，当前版本：v{}，请通过前端地址访问。",
            docBaseConfig.getName(), docBaseConfig.getVersion());
    }
    ...
    public ResponseDTO<Void> register(@RequestBody AddUserCommand command) {
        if (!loginService.isRegisterEnabled()) {
        ...
        if (command.getNickname() == null || command.getNickname().isBlank()) {
            command.setNickname(command.getUsername());
        }
